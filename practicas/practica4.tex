...
\begin{tikzpicture}[x=1cm,y=1cm, every node/.style={font=\sffamily\small}]
  ...
  \draw[rounded corners=2pt, color=uzaLine, fill=uzaCodeBg] (-4.65,-0.35) rectangle (4.65,2.8);
  \draw[-{Latex[length=2mm]}, color=uzaMuted] (-4.25,0) -- (4.35,0) node[right] {$x$};
  \draw[-{Latex[length=2mm]}, color=uzaMuted] (0,-0.18) -- (0,2.55) node[above] {$y$};
  \draw[domain=-4:4,samples=180,smooth,variable=\x,color=uzaBlue,thick] plot ({\x},{\catScale*(\catA*(exp(\x/\catA)+exp(-\x/\catA))/2)});
  \draw[color=uzaWarn,thick,dashed] (-4,{\catScale*\catB}) -- (4,{\catScale*\catB}) node[right,color=uzaWarn] {$y=b$};
