...
### El código fuente: programación orientada a objetos
...
4. Pulsa **`F6`** unas pocas veces para avanzar paso a paso y observa cómo se va construyendo la matriz de proyección.
5. Pulsa **`F8`** para continuar la ejecución normal del programa.
6. ¿Por qué el programa vuelve una y otra vez a esa misma línea aunque no hayas hecho nada?
7. Cuando termines, desactiva el *breakpoint*: botón derecho → **Disable Breakpoint**.
:::
...
::: {.callout-note}
## Comprueba la rotación de la cámara
...
Recuerda que las clases que debemos utilizar son `geometria.Punto` y `geometria.Matriz4x4`. Para crear la matriz de rotación, puedes usar el método `rotacionZ` de `Matriz4x4`. Para multiplicar el punto por la matriz, puedes usar el método `multiplicar` de `Matriz4x4`. Los ángulos de rotación se expresan en radianes, así que para convertir 90° a radianes puedes multiplicar el ángulo por `Math.PI / 180.0`.
...
- O bien, muestra por pantalla los valores del punto resultante. Por ejemplo, utiliza `System.out.println(res.elemento(0) + ", " + res.elemento(1) + ", " + res.elemento(2));` para imprimir el resultado en la consola de *Eclipse*.
:::
...
<div id="phong-demo-uza" style="margin:1.2rem 0; display:grid; grid-template-columns:1fr 300px; gap:1rem; align-items:start;">
  ...
  <div style="background:#fff; border:1px solid #e2e8f0; border-radius:14px; padding:1rem; box-shadow:0 4px 16px rgba(0,0,0,0.06); display:flex; flex-direction:column; gap:0.7rem;">
    ...
    <div style="display:grid; grid-template-columns:118px 1fr 44px; gap:0.4rem 0.6rem; align-items:center; font-size:0.88rem;">
      ...
      <input id="ph-ka" type="range" min="0" max="1" step="0.02" value="0.02" style="width:100%;">
      <span id="ph-ka-v" style="text-align:right; font-variant-numeric:tabular-nums;">0.02</span>

      <label><span style="font-family:Georgia, 'Times New Roman', serif;"><em>k</em><sub>d</sub></span> difuso</label>
      <input id="ph-kd" type="range" min="0" max="1" step="0.02" value="0.1" style="width:100%;">
      <span id="ph-kd-v" style="text-align:right; font-variant-numeric:tabular-nums;">0.1</span>
...
<small class="img-compare-caption">Completo ↔ sin componente especular. Por ejemplo, fíjate en el brillo del saxofón.</small>
:::
...
<small class="img-compare-caption">Ambiente + difusa ↔ solo difusa.</small>
:::

:::

<p class="img-compare-credits">Comparación de términos <em>k</em><sub>a</sub>, <em>k</em><sub>d</sub> y <em>k</em><sub>s</sub> sobre una escena de <a href="https://www.blenderkit.com/get-blenderkit/1ac8808f-11e6-4c76-894e-227f1eb3cba5/" target="_blank" rel="noopener">BlenderKit</a>, autor: Russo 3D.</p>
...
::: {.callout-note}
## Reto de visualización: modo ondas
...
<div class="phong-bsdf-map-uza" aria-label="Mapa visual de componentes difusa y especular en Phong">
  ...
  <div class="pbm-shell">
    <div class="pbm-head">
      <div>
        <p class="pbm-title">Mapa visual: de superficie mate a reflejo metálico</p>
        <p class="pbm-subtitle">Phong no es un BSDF físico, pero sus parámetros sirven para imitar estos comportamientos.</p>
      </div>
    ...
    <div class="pbm-flow">
      <section class="pbm-side" aria-label="Ejemplos de componente difusa">
        <figure class="pbm-card">
          ...
          <figcaption>Casi todo se entiende con <code>kd</code>; <code>ks</code> sería bajo.</figcaption>
        </figure>

        <figure class="pbm-card">
          <img src="../resources/practica9/images/mitsuba_bsdf_pplastic_diffuse_component.jpg" alt="Componente difusa de un material plástico renderizado en Mitsuba">
          <span class="pbm-badge diff">Base plástica</span>
          <figcaption>Solo la parte difusa del plástico; su brillo esta a la derecha.</figcaption>
        </figure>
      </section>
      ...
      <section class="pbm-core" aria-label="Lectura de parametros Phong">
        <div class="pbm-node">
          ...
          <p><code>ka</code> levanta la escena, <code>kd</code> da cuerpo mate y <code>ks</code> añade el destello.</p>
        </div>

        <div class="pbm-note">
          <p><strong>Plástico:</strong> base difusa (<code>kd</code>) + capa especular (<code>ks</code>).</p>
          <p><strong>Metal:</strong> domina la parte especular y el reflejo también toma color.</p>
        </div>
      </section>
      ...
      <section class="pbm-side" aria-label="Ejemplos de componente especular">
        ...
        <figure class="pbm-card">
          <img src="../resources/practica9/images/mitsuba_bsdf_pplastic_specular_component.jpg" alt="Componente especular de un material plástico renderizado en Mitsuba">
          <span class="pbm-badge spec">Brillo plástico</span>
  ...
  <p class="pbm-credit">
    Imágenes: documentación de <a href="https://mitsuba.readthedocs.io/en/latest/src/generated/plugins_bsdfs.html">Mitsuba 3, BSDF plugins</a>. Usadas aquí como referencia visual cualitativa para los terminos de Phong.
  </p>
</div>
```-->
