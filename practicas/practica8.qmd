...
Cuando se diseña un edificio no solo se define su forma: también se decide qué elementos lo componen, cómo se relacionan entre sí y qué información será útil más adelante, durante la obra y una vez terminado. El **modelado de información de construcción** (**BIM**, *Building Information Modeling*) nace precisamente para organizar todo eso dentro de una representación digital coherente.

Por eso un modelo BIM no es simplemente una maqueta 3D. Además de la geometría, puede incorporar materiales, identificadores, relaciones espaciales, propiedades técnicas y otra información relevante para distintas fases del proyecto. La gracia está en que el modelo deja de ser solo algo que "se visualiza" y pasa a ser también algo que "se consulta", "se coordina" y "se reutiliza".

Esta forma de trabajar resulta especialmente útil porque concentra en un mismo entorno información que, de otro modo, estaría repartida entre planos, tablas, memorias y documentos técnicos. En lugar de revisar cada pieza por separado, BIM permite entender mejor el edificio como un sistema formado por objetos relacionados entre sí.

En el trabajo profesional, ese modelo acompaña al edificio a lo largo de buena parte de su **ciclo de vida**. Puede ayudar en fases tempranas de diseño y coordinación, servir para detectar interferencias antes de construir, apoyar la planificación de obra y seguir siendo útil después para mantenimiento, inventario o explotación. Es decir, la misma base de información puede ir cambiando de papel a medida que avanza el proyecto.
...
Aquí es donde encaja bien el **LiDAR** (*Light Detection and Ranging*). Esta técnica emite pulsos láser y estima la distancia a partir del tiempo que tarda la luz en regresar, generando así una **nube de puntos** tridimensional muy densa. En lugar de partir de un modelo ideal dibujado desde cero, el LiDAR permite capturar el estado real de un edificio o de su entorno.

Esa conexión con BIM es especialmente interesante en fases avanzadas del ciclo de vida. Por ejemplo, en rehabilitación, patrimonio o control de obra puede interesar más describir lo que existe realmente que lo que se proyectó en su día. A partir de la nube de puntos, en flujos de trabajo conocidos como ***scan-to-BIM***, se genera un modelo *as-built* que sirve para documentar, comparar, medir o actualizar la información del edificio.

Aunque en esta práctica no construiremos un modelo BIM, sí conviene tener presente esa relación: BIM organiza la información del edificio, y técnicas como LiDAR ayudan a capturarla cuando necesitamos partir de la realidad física.
...
<div class="lidar-viewer">
  <div class="lidar-viewer-pane">
    ...
    <span class="lidar-viewer-label">Nube de puntos (artificial)</span>
  </div>
  <div class="lidar-viewer-pane">
    <div class="lidar-viewer-canvas" id="lidar-viewer-mesh"></div>
    <span class="lidar-viewer-label">Malla generada a partir de nube de puntos</span>
  </div>
  <div class="lidar-viewer-pane lidar-viewer-pane-wide">
    <div class="lidar-viewer-canvas lidar-viewer-canvas-wide" id="lidar-viewer-room"></div>
    <span class="lidar-viewer-label">Nube de puntos: habitación</span>
  </div>
</div>
...
function createViewer(id, url, mode, options = {}) {
  ...
  // Brighter ambient for textured meshes; directional adds shape definition.
  scene.add(new THREE.HemisphereLight(0xffffff, 0x444466, mode === 'points' ? 1.0 : 1.4));
  const dir = new THREE.DirectionalLight(0xffffff, mode === 'points' ? 0.6 : 0.8);
  ...
  (async () => {
    try {
      ...
      if (ext === 'glb' || ext === 'gltf') {
        // GLB/glTF: self-contained geometry + materials + textures.
        const gltf = await new Promise((resolve, reject) => {
          new GLTFLoader().load(url, resolve, undefined, reject);
        });
        const root = gltf.scene;

        // Center and normalize so max dimension = 1.5 world units.
        ...
        root.position.copy(center).multiplyScalar(-scaleFactor);
        root.updateMatrixWorld(true);

        scene.add(root);
        console.log('[lidar-viewer]', id, { format: ext, mode });
      ...
      // PLY path: fetch buffer, parse, optionally load texture from `comment TextureFile`.
      ...
      let textureFiles = [];
      if (mode !== 'points') {
        const headSlice = buffer.slice(0, Math.min(8192, buffer.byteLength));
        const headerText = new TextDecoder().decode(headSlice);
        const endIdx = headerText.indexOf('end_header');
        if (endIdx !== -1) {
          const headerOnly = headerText.slice(0, endIdx);
          const re = /^\s*comment\s+TextureFile\s+(.+?)\s*$/gm;
          let m;
          while ((m = re.exec(headerOnly)) !== null) textureFiles.push(m[1]);
        }
      }
      ...
      if (textureFiles.length > 0) {
        ...
        const texUrl = baseDir + textureFiles[0];
        try {
          ...
          if (textureFiles.length > 1) {
            console.warn('[lidar-viewer] ' + id + ': PLY references ' + textureFiles.length +
              ' textures; only the first is applied. Convert to a single-atlas mesh or .glb for full coverage.');
          }
        } catch (e) {
          console.warn('[lidar-viewer] ' + id + ': texture load failed for ' + texUrl, e);
        }
      }
      ...
      console.log('[lidar-viewer]', id, { format: 'ply', vertCount, hasColor, hasUv, textureFiles, mode });
      ...
      if (mode === 'points') {
        ...
        const useTex = texture && hasUv;
        const material = new THREE.MeshStandardMaterial({
          map: useTex ? texture : null,
          color: useTex ? 0xffffff : (hasColor ? 0xffffff : 0xc0c8d4),
          vertexColors: !useTex && hasColor,
...
createViewer('lidar-viewer-mesh',  '../resources/practica8/models/goya_mesh.glb',        'mesh');
createViewer('lidar-viewer-cloud', '../resources/practica8/models/goya_point_cloud.ply', 'points');
createViewer('lidar-viewer-room',  '../resources/practica8/models/room.ply',             'points', { startInside: true });
</script>
```
...
*Visor interactivo de malla y nube de puntos disponible en la versión HTML.*
:::
