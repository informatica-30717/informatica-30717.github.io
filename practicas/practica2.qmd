...
![](../resources/lesson2/overview.svg){width=100%}
:::
...
::: {style="width: 7%; min-width: 50px; flex-shrink: 0;"}
![](../resources/lesson2/tutorials.svg){width=100%}
:::
...
::: {style="width: 7%; min-width: 50px; flex-shrink: 0;"}
![](../resources/lesson2/samples.svg){width=100%}
:::

::: {.ms-3}
**Samples**: otra manera de conocer cómo funciona la herramienta es ejecutar algunos de los ejemplos que Eclipse pone a nuestra disposición.
:::
...
::: {style="width: 7%; min-width: 50px; flex-shrink: 0;"}
![](../resources/lesson2/new.svg){width=100%}
:::
...
::: {style="width: 7%; min-width: 50px; flex-shrink: 0;"}
![](../resources/lesson2/workbench.png){width=100%}
:::

::: {.ms-3}
**Workbench**: usando esta opción se accede directamente al espacio de trabajo de Eclipse.
:::
...
Si cerramos la pestaña de bienvenida (*Welcome*, arriba a la izquierda), nos conducirá al entorno de trabajo que se explica en la siguiente sección.
...
Los proyectos se crean a través de la opción `File > New > Java Project`. Para crear el nuevo proyecto hay que dotarlo de un nombre identificativo (por ejemplo, `practica2`). Dicho proyecto se guarda en un directorio dentro del equipo, y dentro del *workspace* seleccionado al lanzar *Eclipse*. Conviene seleccionar la opción ***Create separate source and output folders***. En la @fig-new_project se pueden observar todas las propiedades que se pueden definir en la creación del proyecto.

El nuevo proyecto que has creado se configurará sobre la carpeta:

`C:\practicasARQ\practica2`

Abre dicha carpeta en el explorador de archivos del sistema operativo y observa que en la carpeta `practica2` se han creado dos subcarpetas. En src se almacenarán los ficheros con el código en lenguaje Java (*source*), mientras que en la carpeta *bin* se almacenan los ficheros binarios (*binaries*).

![Propiedades que debemos configurar al crear un nuevo proyecto.](../resources/lesson2/new_project.png){#fig-new_project}
...
Para añadir nuevas clases al proyecto utilizaremos la opción de menú `File > New > Class`. Selecciona `src` como subcarpeta e introduce `HolaMundo` como nombre del archivo que contendrá el programa (clase) que deberás escribir a continuación. La opción `public static void main(String[] args)` debe marcarse para que la clase sea ejecutable (ejecutándose por tanto el método *main* de la clase). La @fig-new_class muestra la configuración que se debe realizar para crear una nueva clase Java.

![Creación de una nueva clase de Java en *Eclipse IDE*.](../resources/lesson2/new_class.png){#fig-new_class}
...
Guarda los cambios que hemos introducido pulsando los iconos con disquetes que hay en la barra de herramientas superior (![](../resources/lesson2/save.svg){width=1em}![](../resources/lesson2/save_all.svg){width=1em}), pulsando `Ctrl-S` o bien con `File > Save`.
