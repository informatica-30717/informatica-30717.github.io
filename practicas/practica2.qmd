...
![Configuración del *workspace*.](../resources/practica2/workspace_eclipse.png){#fig-workspace_eclipse}
...
Los proyectos se crean a través de la opción `File > New > Java Project`. Para crear el nuevo proyecto hay que dotarlo de un nombre identificativo (por ejemplo, `practica2`). Dicho proyecto se guarda en un directorio dentro del equipo, en el *workspace* seleccionado al lanzar *Eclipse*. Conviene seleccionar la opción ***Create separate source and output folders***. En la @fig-new_project se pueden observar todas las propiedades que se deben definir en la creación del proyecto.
...
Teniendo en cuenta que nuestro entorno de trabajo se encuentra en `C:\practicasARQ`, el nuevo proyecto que has creado se configurará sobre la carpeta:
...
Fijaos en que, además de la carpeta, hemos indicado el nombre de un paquete. Dicho nombre no se puede dejar en blanco, por lo que hemos escrito `practica2`. En futuras sesiones de prácticas hablaremos de qué es un paquete, y cómo crearlo para estructurar las clases que creemos en cada proyecto.
