...
3. Se repite el proceso con el siguiente número primo en la lista hasta que se hayan procesado todos los números.
...
**Anota el tiempo de ejecución en Microsoft Excel para cada una de las implementaciones, y cuatro valores de $n$ diferentes**: $10^4$, $10^5$, $10^6$ y $10^7$. Los valores de $n$ se representarán en el eje $X$ de un gráfico, y el tiempo de ejecución en el eje $Y$. Dibuja una gráfica similar a la ilustrada a continuación:
...
// Llama a tu método de la criba de Eratóstenes aquí
...
```{ojs}
// define a custom slider function
...
viewof N = cleanSlider([10, 50], { value: 30, step: 1, label: "Valor N (Sin optimizar): " })
```
...
steps2 = {
  ...
  // 1. Modificado: p llega hasta max en lugar de detenerse en la raíz cuadrada
  for (let p = 2; p <= max; p++) {
    if (state[p] === "candidate") {
      out.push({ type: "prime", p, state: [...state] });

      // 2. Modificado: k empieza en p * 2 en lugar de p * p para mostrar redundancia
...
viewof step2 = cleanSlider([0, maxStep2], {
  value: 0,
  step: 1,
  label: "Paso de la criba: "
})
```
...
```{ojs}
{
  ...
  return html`
    <div style="margin-top:15px;">
      ${Plot.plot({
        height: Math.ceil(max / cols) * 35 + 20, 
        margin: 10,
        x: { axis: null },
      ...
      <div style="margin-top:15px; font-size: 1.1em; height: 1.5em;">
        ${
          s.type === "prime"
            ? html`🔎 Probando primo <b>${currentPrime}</b>`
            : s.type === "discard"
            ? html`❌ ${lastDiscard} es múltiplo de <b>${currentPrime}</b>`
            : html`✅ Criba completada`
        }
      </div>
    </div>
  `;
...
**En la solución previa se itera hasta `n` para encontrar los números primos, lo cual es ineficiente**. En esta solución, **se optimiza el proceso iterando solo hasta la raíz cuadrada de `n`**, ya que si un número `n` tiene un factor mayor que su raíz cuadrada, entonces necesariamente tendrá un factor menor que la raíz cuadrada.
...
**Corolarios**: Si $n$ no es primo, uno de sus factores $d$ cumple $1 ≤ d ≤ \sqrt{n}$. Si $n$ no tiene factores $d$ con $1 ≤ d ≤ \sqrt{n}$, entonces es primo. Por tanto, $n$ es primo si y solo si es divisible por algún primo menor o igual a $\sqrt{n}$.
:::
...
```{ojs}
// use it for the N parameter
viewof N2 = cleanSlider([10, 50], { value: 30, step: 1, label: "Valor N: " })

// compute the steps of the sieve
steps = {
  const max = N2;
  let state = Array(max + 1).fill("candidate");
  state[0] = state[1] = "discarded";
...
// use it for the step timeline
viewof step = cleanSlider([0, maxStep], {
  value: 0,
  step: 1,
  label: "Paso de la criba: "
})
...
```{ojs}
// reusing cleanSlider from the first block
viewof N3 = cleanSlider([10, 150], { value: 60, step: 1, label: "Valor N: " })

steps3 = {
  const max = N3;
  if (max < 3) return [{ type: "final", state: [] }];

  // 1. Array size: floor((n-3)/2) + 1
  const size = Math.floor((max - 3) / 2) + 1;
  let state = Array(size).fill("candidate");

  const out = [];

  // 2. Loop only over odds: p = 2i + 3
  for (let i = 0; ; i++) {
    let p = 2 * i + 3;
    if (p * p > max) break;
    ...
    if (state[i] === "candidate") {
      ...
      // jump by 2p to only hit odd multiples
      for (let mult = p * p; mult <= max; mult += 2 * p) {
        let idx = (mult - 3) / 2; // convert value back to array index
...
maxStep3 = steps3.length - 1

viewof step3 = cleanSlider([0, maxStep3], {
  value: 0,
  step: 1,
  label: "Paso de la criba: "
})
{
  if (!steps3 || steps3.length === 0) return html`<div>Cargando...</div>`;

  const safeStep = Math.min(step3, steps3.length - 1);
  const s = steps3[safeStep];
  ...
  const size = Math.floor((max - 3) / 2) + 1;
  const cols = Math.ceil(Math.sqrt(size)); // The grid will visibly be smaller!
  
  const currentPrime = s.p;
  const lastDiscard = s.k;

  const grid = Array.from({ length: size }, (_, i) => {
    const num = 2 * i + 3; // Decode the actual number from the index
    let bgColor = "#2a9d8f"; 
  ...
  return html`
    <div style="margin-top:15px;">
      <div style="margin-bottom:10px; font-size: 0.95em; color: #666;">
        <i>Nota: El número 2 es primo implícito. El vector solo contiene posiciones para $\lfloor(N-3)/2\rfloor + 1$ impares.</i>
      </div>

      ${Plot.plot({
        height: Math.ceil(size / cols) * 35 + 20, 
        margin: 10,
        x: { axis: null },
        y: { axis: null, reverse: true },
        color: { type: "identity" }, 
      ...
      <div style="margin-top:15px; font-size: 1.1em; height: 1.5em;">
        ${
          s.type === "prime"
            ? html`🔎 Probando primo impar <b>${currentPrime}</b>`
            : s.type === "discard"
            ? html`❌ ${lastDiscard} es múltiplo impar de <b>${currentPrime}</b>`
            : html`✅ Criba completada (Tamaño del vector: ${size} posiciones)`
        }
      </div>
    </div>
  `;
